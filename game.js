...
while (player1TotalCards < totalWinningCards || player2TotalCards < totalWinningCards) {
  if (this.compare(this.player1.hand[i].value, this.player2.hand[i].value) === 0) {
    // play war
    console.log("Playing War");

  } else if (this.compare(this.player1.hand[this.player1.hand.length -1].value, 
    this.player2.hand[this.player2.hand.length - 1].value,) === 1) {
// 1. player with the highest card takes both cards 
// 2. puts them face down on top of his stack
...
this.player2.hand.splice(i, 1);
      ...
      this.player2.hand.push(this.player1.hand[this.player1.hand.length -1].value,);
